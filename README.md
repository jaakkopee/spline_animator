...
The audio-reactive system:
1. Loads the audio file using librosa (supports WAV, MP3, FLAC, OGG, etc.)
2. Extracts audio features:
   - Energy envelope (loudness over time)
   - Spectral centroid (brightness of the sound)
   - Zero-crossing rate (noisiness)
   - Onset events (beat-like transients)
   - Chroma features (harmonic content)
3. Detects segment boundaries using onset detection
4. Maps audio features to timeline segment parameters:
   - **Frame count**: Driven by energy. Louder sections get more frames (slower motion)
   - **Easing**: Determined by energy and spectral content
     - High energy + bright: `ease-out` (sharp easing)
     - Moderate energy: `ease-in-out` (smooth easing)
     - Low energy: `ease-in` or `linear` (gentle easing)
   - **Spline tension**: Scaled by energy (-1 to +1)
     - Low energy: negative tension (loose, flowing curves)
     - High energy: positive tension (tight, controlled curves)
   - **Spline endpoint**: `clamp` for low energy, `mirror` for high energy
   - **Continuity**: Adjusted based on energy to shape transition smoothness

### Options

- `--audio`: Path to audio file (required)
- `--output`: Output timeline JSON path (required)
- `--keyframes`: Comma-separated keyframe paths. If fewer keyframes than segments are detected, they cycle through. If omitted, generates placeholder names.
...
- M4A
- AIFF
- And any format supported by audioread
